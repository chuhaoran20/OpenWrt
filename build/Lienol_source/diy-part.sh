...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='10.10.10.51'" $ZZZ                             # IPv4 地址(openwrt后台地址)
sed -i "/uci commit network/i\uci set network.lan.netmask='255.255.255.0'" $ZZZ                           # IPv4 子网掩码
sed -i "/uci commit network/i\uci set network.lan.gateway='10.10.10.1'" $ZZZ                            # IPv4 网关
#sed -i "/uci commit network/i\uci set network.lan.broadcast='10.10.10.255'" $ZZZ                          # IPv4 广播
sed -i "/uci commit network/i\uci set network.lan.dns='10.10.10.1'" $ZZZ                                # DNS(多个DNS要用空格分开)
...
sed -i "s/OpenWrt /${Author} Compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ                  # 增加个性名字Yufeng
